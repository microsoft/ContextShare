...
# PromptVault - Complete Setup Guide

This guide will walk you through everything you need to know to set up and use the PromptVault extension effectively.
...
code --install-extension ./promptvault-0.1.32.vsix --force
```
...
3. **Click the icon** to open the PromptVault view

## Understanding the Interface

### Activity Bar Icon 📚
- Located in the left sidebar
- Shows status: "PromptVault 📚 X/Y" (X active resources out of Y total)
...
Access settings via: **File** → **Preferences** → **Settings** → Search "PromptVault"
...
1. **Open Settings** → Search "PromptVault"
...
2. **Team Members**:
  ...
  # Open VS Code, go to PromptVault view
...
2. **Configure path**: Go to Settings → PromptVault → Catalog Directory
...
**Check logs**: 
- Output panel → "PromptVault" channel
- If file logging enabled: User global storage location shown in diagnostics

### Getting Help

1. **Check Output Panel**: View → Output → "PromptVault"
