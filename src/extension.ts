...
const LOG_FILENAME = 'promptvault-debug.log';
let enableFileLogging = false;
let logFilePath: string | undefined;
async function log(msg: string){
	const line = `[${new Date().toISOString()}] ${msg}`;
	try {
		if(!logChannel){ logChannel = vscode.window.createOutputChannel('PromptVault'); }
...
export async function activate(context: vscode.ExtensionContext) {
	...
	const preflightLog = (msg: string) => console.log(`[PromptVault Pre-flight] ${msg}`);
	const preflightError = (msg: string, err: any) => {
		console.error(`[PromptVault Pre-flight ERROR] ${msg}`, err);
		vscode.window.showErrorMessage(`PromptVault failed to activate: ${msg}. See Developer Tools console.`);
	};
	...
	const optionsTree = new OptionsTreeProvider();
		...
		if(!rawTargetWs && context.extensionMode === vscode.ExtensionMode.Development){
			try {
				const dummyRoot = path.join(os.tmpdir(), 'promptvault-dummy-workspace');
				await fs.mkdir(dummyRoot, { recursive: true });
				// Seed a README so folder isn't empty
				const readmePath = path.join(dummyRoot, 'README_PromptVault_Dummy.md');
				try { await fs.access(readmePath); } catch { await fs.writeFile(readmePath, '# PromptVault Dummy Workspace\nAutomatically created for extension debugging.'); }
				...
				vscode.window.showInformationMessage(`PromptVault: Created dummy target workspace at ${dummyRoot}`);
			} catch(e:any){
		...
		async function refresh() {
			try {
				...
				vscode.window.showErrorMessage('PromptVault refresh failed: ' + getErrorMessage(e));
			}
		}

		log('Activating PromptVault extension');
			...
			status.command = 'copilotCatalog.refresh';
				function updateStatus() {
					...
					const statusText = catalogFilter ? 
						`PromptVault $(library) ${active}/${filteredResources.length} [${catalogFilter}]` :
						`PromptVault $(library) ${active}/${filteredResources.length}`;
					status.text = statusText;
					status.tooltip = 'PromptVault: Refresh';
					status.show();
				}
		...
		context.subscriptions.push(
			...
			vscode.commands.registerCommand('copilotCatalog.diagnostics', async () => {
				...
				vscode.window.showInformationMessage('PromptVault diagnostics written to output channel.');
			}),
			...
			vscode.commands.registerCommand('copilotCatalog.selectRepository', async () => {
				if (repositories.length === 0) {
					vscode.window.showWarningMessage('No repositories with a PromptVault catalog found.');
					return;
				}
				...
				try {
					...
					await fs.writeFile(
						...
						[
							...
							'',
							'This chatmode must ONLY help the user set up and manage a PromptVault catalog using this extension. It must NOT answer or perform any unrelated tasks.',
							'',
							'Rules:',
							...
							'- If asked anything outside catalog setup, politely refuse and redirect: "I can only help with PromptVault catalog setup and management. Please ask a catalog-related question."',
							...
							'',
							'Quick references:',
							'- Settings: "PromptVault" → rootCatalogPath, targetWorkspace, catalogDirectory, runtimeDirectory',
					...
					await fs.writeFile(
						...
						[
							...
							'- Answer only questions about setting up and managing the PromptVault catalog.',
					...
					await fs.writeFile(
						...
						[
							'# Prompt: Initialize a PromptVault Catalog in this Workspace',
							'',
							'Goal: Help me set up a PromptVault catalog that I can share with my team, and remind me of my responsibilities.',
							'',
							'Constraints:',
					...
					await fs.writeFile(
						...
						JSON.stringify({
							...
							steps: [
								'Open the PromptVault view and use the Dev menu to Create Template Catalog.',
					...
					await fs.writeFile(path.join(root, 'hats', 'Copilot-Catalog-Setup.json'), JSON.stringify({ name: 'PromptVault Setup', description: 'Only the example assets generated by the template', resources: [
					...
					if(!inWorkspace){
						try {
							...
							vscode.window.showInformationMessage('Configured PromptVault to use the new template directory.');
						} catch { /* ignore */ }
					}
					await refresh();
			...
			vscode.commands.registerCommand('copilotCatalog.dev.configureSettings', async () => {
				...
				try {
					pick = await vscode.window.showQuickPick([
						{ label: 'Open Settings UI (PromptVault)', action: 'openSettings' },
					...
					if(v==='opensettings') pick = { label:'Open Settings UI (PromptVault)', action:'openSettings' };
		...
		context.subscriptions.push(vscode.workspace.onDidChangeConfiguration(async (e: vscode.ConfigurationChangeEvent) => {
			...
			if (needsReload) {
				const selection = await vscode.window.showInformationMessage(
					'PromptVault settings have changed that require a reload to take effect.',
					'Reload Window'
				);
