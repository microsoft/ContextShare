...
import { Preset, Repository, Resource, ResourceCategory, ResourceState } from './models';
...
await loadResources();
// No-op: presets are discovered on demand when command is invoked
...
await vscode.workspace.fs.createDirectory(vscode.Uri.file(path.join(root, 'presets')));
...
'',
'Rules:',
'- Scope strictly to catalog setup: scaffolding folders, configuring settings, activating/deactivating resources, understanding Presets, and packaging/installation steps.',
...
'- Presets: presets to activate/deactivate groups of resources',
].join('\n')
);
await fs.writeFile(
...
'3) How to activate/deactivate a resource and apply a Preset',
...
'Create/apply a Preset to quickly activate a set of resources.',
...
await fs.writeFile(path.join(root, 'presets', 'Copilot-Catalog-Setup.json'), JSON.stringify({ name: 'ContextShare Setup', description: 'Only the example assets generated by the template', resources: [
...
vscode.commands.registerCommand('copilotCatalog.dev.scanGitRepository', async () => {
  // Enhanced: branch enumeration + commit lock option
  ...
  let branches: Array<{ label: string; description?: string; detail?: string; _name: string; _commit: string }> = [];
  try {
    await gitCatalogService.init([], context.globalStorageUri.fsPath);
    const list = await gitCatalogService.listRemoteBranches(url);
    branches = list.map(b => ({
      label: (b.name === 'main' || b.name === 'master') ? `$(git-branch) ${b.name}` : b.name,
      description: b.commit.substring(0,8),
      detail: 'Remote branch',
      _name: b.name,
      _commit: b.commit
    }));
  } catch (e:any) {
    await logger.warn(`Branch enumeration failed (fallback to manual): ${getErrorMessage(e)}`);
  }
  ...
  if (branches.length) {
    const pick = await vscode.window.showQuickPick(
      [
        ...branches,
        { label: '$(edit) Enter branch manually…', description: 'Manual branch name', _name: '__manual__', _commit: '' }
      ],
      { placeHolder: 'Select branch (or choose manual entry)' }
    );
    if (!pick) return;
    if (pick._name === '__manual__') {
      const manual = await vscode.window.showInputBox({ prompt: 'Branch name', value: 'main' });
      if (!manual) return;
      chosenBranch = manual.trim();
    } else {
      chosenBranch = pick._name;
      chosenHeadCommit = pick._commit;
    }
  } else {
    const manual = await vscode.window.showInputBox({ prompt: 'Branch name (unable to enumerate, defaults to main)', value: 'main' });
    if (!manual) return;
    chosenBranch = manual.trim();
  }

  // Decide tracking vs locking
  let lockedCommit: string | undefined;
  if (chosenHeadCommit) {
    const mode = await vscode.window.showQuickPick([
      { label: `Track branch "${chosenBranch}" (auto updates)`, detail: 'Always latest on refresh', _mode: 'track' },
      { label: `Lock to current commit ${chosenHeadCommit.substring(0,8)}`, detail: 'Pinned until changed manually', _mode: 'lock-head' },
      { label: 'Lock to another commit…', detail: 'Enter a specific commit SHA', _mode: 'lock-other' }
    ], { placeHolder: 'Select tracking mode' });
    if (!mode) return;
    if (mode._mode === 'lock-head') lockedCommit = chosenHeadCommit;
    else if (mode._mode === 'lock-other') {
      const sha = await vscode.window.showInputBox({
        prompt: 'Commit SHA to lock to',
        validateInput: v => /^[0-9a-f]{7,40}$/i.test(v.trim()) ? undefined : 'Enter a valid SHA (7-40 hex)'
      });
      if (!sha) return;
      lockedCommit = sha.trim();
    }
  } else {
    const mode = await vscode.window.showQuickPick([
      { label: `Track branch "${chosenBranch}"`, _mode: 'track' },
      { label: 'Lock to specific commit…', _mode: 'lock-other' }
    ], { placeHolder: 'Select tracking mode' });
    if (!mode) return;
    if (mode._mode === 'lock-other') {
      const sha = await vscode.window.showInputBox({
        prompt: 'Commit SHA to lock to',
        validateInput: v => /^[0-9a-f]{7,40}$/i.test(v.trim()) ? undefined : 'Invalid SHA'
      });
      if (!sha) return;
      lockedCommit = sha.trim();
    }
  ...
  try {
    await logger.info(`Adding Git repository: ${url} branch=${chosenBranch} lock=${lockedCommit ? lockedCommit.substring(0,8) : '(none)'}`);
    vscode.window.showInformationMessage('Scanning repository for catalogs...');

    // Ensure state (re-init best effort)
    await gitCatalogService.init([], context.globalStorageUri.fsPath);

    // Add remote (with optional lock)
    await gitCatalogService.addRemoteInteractively(url, chosenBranch || 'main', lockedCommit);
    ...
    if (catalogs.length === 0) {
      vscode.window.showInformationMessage('No catalogs found in repository.');
      return;
    }
 ...
 // Refresh all previously added Git repositories (interactive updates)
 vscode.commands.registerCommand('copilotCatalog.dev.refreshGitRepositories', async () => {
  try {
    ...
    // Prune unused catalogs before refreshing
    const config = vscode.workspace.getConfiguration();
    const catalogDirectories = config.get<Record<string, string>>('copilotCatalog.catalogDirectory', {});
    const activeCatalogPaths = Object.keys(catalogDirectories);
    await gitCatalogService.pruneUnusedCatalogs(activeCatalogPaths);
    ...
    for (const remote of remotes) {
      // Locked: ensure present/scan once, never auto update
      if (remote.lockedCommit) {
        lockedCount++;
        if (!remote.lastCommit) {
          // First-time scan for locked repo
          try { await gitCatalogService.updateTrackedRemote(remote); } catch {}
        }
        continue;
      }

      // First time (no lastCommit) -> update silently
      if (!remote.lastCommit) {
        const ok = await gitCatalogService.updateTrackedRemote(remote);
        if (ok) updatedCount++;
        continue;
      }

      // Detect update
      const info = await gitCatalogService.detectBranchUpdate(remote);
      if (info.hasUpdate && info.remoteHead) {
        const choice = await vscode.window.showQuickPick([
          { label: `Update ${remote.url}@${remote.branch} → ${info.remoteHead.substring(0,8)}`, _act: 'update' },
            { label: `Skip (stay at ${info.current?.substring(0,8)})`, _act: 'skip' },
            { label: `Lock to ${info.remoteHead.substring(0,8)}`, _act: 'lock' }
        ], { placeHolder: `Update available for ${remote.url}@${remote.branch}` });
        if (!choice) { skippedCount++; continue; }
        if (choice._act === 'update') {
          const ok = await gitCatalogService.updateTrackedRemote(remote);
          if (ok) updatedCount++; else skippedCount++;
        } else if (choice._act === 'skip') {
          skippedCount++;
        } else if (choice._act === 'lock') {
          await gitCatalogService.addRemoteInteractively(remote.url, remote.branch, info.remoteHead);
          updatedCount++;
        }
      }
    }
    ...
    if (changed) {
      ...
      await logger.info(`Added ${Object.keys(catalogMap).length} catalog path(s) (merged).`);
    }
    ...
    vscode.window.showInformationMessage(`Git refresh done. Updated:${updatedCount} Skipped:${skippedCount} Locked:${lockedCount}`);
    await logger.info(`Git refresh complete: updated=${updatedCount} skipped=${skippedCount} locked=${lockedCount} withCommits=${withCommits}/${updated.length}`);
  } catch(e:any) {
...
vscode.commands.registerCommand('copilotCatalog.dev.listGitRemotes', async () => {
  try {
    await gitCatalogService.ensureLoaded(context.globalStorageUri.fsPath);
    const remotes = gitCatalogService.getRemotes();
    if (remotes.length === 0) {
      vscode.window.showInformationMessage('No Git remotes configured.');
      return;
    }
    ...
    for (const r of remotes) {
      let detail: string;
      if (r.lockedCommit) {
        detail = `locked ${r.lockedCommit.substring(0,8)}`;
      } else if (r.lastCommit) {
        let updateDetail = r.lastCommit.substring(0,8);
        try {
          const info = await gitCatalogService.detectBranchUpdate(r as any);
            if (info.hasUpdate && info.remoteHead) {
              updateDetail = `${r.lastCommit.substring(0,8)} → ${info.remoteHead.substring(0,8)} (update available)`;
            }
        } catch {
      ...
      items.push({
        label: `${r.url} @ ${r.branch}`,
        description: r.lockedCommit ? 'Locked' : 'Tracking',
        detail
      });
    }

    // Display list (no selection action yet; purely informational)
    await vscode.window.showQuickPick(items, { placeHolder: 'Git remotes (status overview)' });
  } catch (e:any) {
    await logger.error('List Git Remotes failed: ' + getErrorMessage(e));
    vscode.window.showErrorMessage('Failed to list Git remotes: ' + getErrorMessage(e));
  }
}),
...
,
// --- Presets (Presets) ---
vscode.commands.registerCommand('copilotCatalog.presets.apply', async () => {
...
vscode.commands.registerCommand('copilotCatalog.presets.createWorkspace', async () => {
if(!currentRepo){ vscode.window.showWarningMessage('No repository available.'); return; }
...
vscode.commands.registerCommand('copilotCatalog.presets.createUser', async () => {
...
vscode.commands.registerCommand('copilotCatalog.presets.delete', async () => {
if(!currentRepo){ vscode.window.showWarningMessage('No repository available.'); return; }
const presets = await presetService.discoverPresets(currentRepo);
